...
def Trainer_L1(opt):
    ...
    for epoch in range(opt.epochs):
        for i, (grayscale, ref, rgb) in enumerate(train_loader):
            ...
            out, q_k_v_list = generator(rgb, grayscale)
            ...
            print("\r[Epoch %d/%d] [Batch %d/%d] [L1 Loss: %.4f] Time_left: %s" %
                ((epoch + 1), opt.epochs, i, len(train_loader), loss_L1.item(), time_left))
            ...
            if (iters_done + 1) % 1000 == 1:
                ...
                utils.save_sample_png(sample_folder = sample_folder, sample_name = 'train_iter%d' % (iters_done + 1), \
...
def Trainer_L1PGAN(opt):
    ...
    for epoch in range(opt.epochs):
        for i, (grayscale, ref, rgb) in enumerate(train_loader):
            ...
            # Overall Loss and optimize
            loss = opt.lambda_l1 * loss_L1 + opt.lambda_p * loss_p + opt.lambda_gan * loss_GAN
            if opt.lambda_tr > 0:
            ...
            if opt.lambda_tr > 0:
                print("\r[Epoch %d/%d] [Batch %d/%d] [L1 Loss: %.4f] [Pecep Loss: %.4f] [Style Loss: %.4f] [GAN Loss: %.4f] [Tri Loss: %.4f] Time_left: %s" %
                    ((epoch + 1), opt.epochs, i, len(train_loader), loss_L1.item(), loss_p.item(), loss_GAN.item(), loss_triple.item(), time_left))
            else:
                print("\r[Epoch %d/%d] [Batch %d/%d] [L1 Loss: %.4f] [Pecep Loss: %.4f] [Style Loss: %.4f] [GAN Loss: %.4f] Time_left: %s" %
                    ((epoch + 1), opt.epochs, i, len(train_loader), loss_L1.item(), loss_p.item(), loss_GAN.item(), time_left))
